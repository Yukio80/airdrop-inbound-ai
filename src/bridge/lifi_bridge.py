...
class LiFiBridge:
    ...
    def _get_arb_balance(self) -> Decimal:
        balance = self.w3_arb.eth.get_balance(self.account.address)
    ...
    def _get_lifi_quote(self, from_chain: str, to_chain: str, from_token: str, to_token: str, amount: Decimal) -> Dict[str, Any]:
        ...
        url = f"https://li.quest/v1/quote?fromChain={from_chain}&toChain={to_chain}&fromToken={from_token}&toToken={to_token}&fromAmount={amount_wei}"
    ...
    def check_and_bridge_if_needed(self, min_eth_balance: float = 0.005, dry_run: bool = False) -> BridgeResult:
        ...
        try:
            ...
            raise RuntimeError(f"Bridge failed: {str(e)}")
